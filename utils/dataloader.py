...
class CRNDataset(Dataset):
    def __init__(self, path_vid, token, dataset, datamode='title+ocr'):
        ...
        vid_df = pd.DataFrame({'video_id': self.vid})
        vid_df['_sort_order'] = range(len(vid_df))
        self.data = self.data_complete.merge(vid_df, on='video_id', how='inner')
        self.data.sort_values('_sort_order', inplace=True)
        self.data.drop(columns=['_sort_order'], inplace=True)
