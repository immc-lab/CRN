...
class Run():
    def __init__(self, config):
        ...
        self.missing_modality = config.get('missing_modality', 'none')
    ...
    def main(self):
        ...
        trainer = Trainer(model=self.model, device=self.device, lr=self.lr,
            dataloaders=dataloaders, epoches=self.epoches, dropout=self.dropout,
            weight_decay=self.weight_decay, epoch_stop=self.epoch_stop,
            save_param_path=self.save_param_dir + self.dataset + "/",
            writer=SummaryWriter(self.path_tensorboard.rstrip('/\\')),
            lambd=self.lambd)
